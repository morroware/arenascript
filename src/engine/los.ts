...
import type { Vec2, EntityId, RobotState, CoverObject } from "../shared/types.js";
...
function pointInRect(p: Vec2, cover: CoverObject): boolean {
...
function rayIntersectsRect(origin: Vec2, dir: Vec2, dist: number, cover: CoverObject): boolean {
  const halfW = cover.width / 2;
  const halfH = cover.height / 2;
...
export function getVisibleEnemies(world: World, robot: RobotState): RobotState[] {
  const visible: RobotState[] = [];
...
export function getVisibleAllies(world: World, robot: RobotState): RobotState[] {
  const visible: RobotState[] = [];
  for (const other of world.robots.values()) {
    if (!other.alive) continue;
