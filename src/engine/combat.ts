...
import type { ActionIntent, RobotState, Vec2, Projectile } from "../shared/types.js";
...
export function resolveCombat(world: World, robot: RobotState, action: ActionIntent | null): void {
...
export function applyDamage(world: World, target: RobotState, damage: number, sourceId: EntityId): void {
...
export function updateCooldowns(world: World): void {
  for (const robot of world.robots.values()) {
    if (!robot.alive) continue;
...
function resolveTargetId(world: World, action: ActionIntent): string | null {
  ...
  if (typeof action.target === "object" && "id" in action.target) {
    return (action.target as { id: string }).id;
  }
  return null;
}

function resolveTargetPosition(world: World, action: ActionIntent): Vec2 | null {
  if (!action.target) return null;
  if (typeof action.target === "object" && "x" in action.target) {
    return action.target as Vec2;
  }
