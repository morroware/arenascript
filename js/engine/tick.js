...
export function runMatch(setup) {
    ...
    for (const participant of setup.participants) {
        const robot = world.spawnRobot(participant.program.robotName, participant.program.robotClass, participant.teamId, participant.program.programId);
    ...
    for (let tick = 0; tick < config.maxTicks; tick++) {
        ...
        for (const [robotId, action] of movementActions)
            replayActions.set(robotId, action);
