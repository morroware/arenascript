...
// ============================================================================
import { distance, normalize, sub, scale, add } from "../shared/vec2.js";
import { CLASS_STATS, ATTACK_DAMAGE, ATTACK_RANGE, ATTACK_COOLDOWN, ATTACK_ENERGY_COST, FIRE_AT_DAMAGE, FIRE_AT_RANGE, FIRE_AT_COOLDOWN, PROJECTILE_SPEED, PROJECTILE_TTL, SHIELD_COOLDOWN, LOW_HEALTH_THRESHOLD, } from "../shared/config.js";
...
export function resolveCombat(world, robot, action) {
    ...
    switch (action.type) {
        case "attack": {
            ...
            if (distance(robot.position, target.position) > range)
                break;
        ...
        case "fire_at": {
            ...
            if (!targetPos)
                break;
            if (distance(robot.position, targetPos) > FIRE_AT_RANGE)
                break;
            const cd = robot.cooldowns.get("attack") ?? 0;
            if (cd > 0)
                break;
...
export function updateProjectiles(world) {
    ...
    for (const [id, proj] of world.projectiles) {
        ...
        proj.ttl--;
        // Check out of bounds
        if (proj.position.x < 0 || proj.position.x > world.config.arenaWidth ||
