...
class ChunkBuilder {
    ...
    emit(op, line = 0, column = 0) {
        ...
        this.code.push(op);
        if (line > 0)
            this.sourceMap.set(offset, { line, column });
        return offset;
    }
    ...
    addConstant(entry) {
        ...
        for (let i = 0; i < this.constants.length; i++) {
            ...
            if (c.type === entry.type) {
                if (c.type === "null")
                    return i;
                if ("value" in c && "value" in entry && c.value === entry.value)
                    return i;
            }
        }
...
export class Compiler {
    ...
    constMap = new Map(); // const name -> constant pool index
    ...
    compileExpression(expr, b) {
        switch (expr.kind) {
            ...
            case "UnaryExpr": {
                ...
                if (expr.operator === "-")
                    b.emit(Op.NEG, expr.span.line, expr.span.column);
                else
                    b.emit(Op.NOT, expr.span.line, expr.span.column);
                break;
            }
            case "ComparisonExpr": {
                this.compileExpression(expr.left, b);
