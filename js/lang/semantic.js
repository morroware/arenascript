...
export class SemanticAnalyzer {
            ...
            for (const entry of program.meta.entries) {
                if (entry.key === "class") {
                    ...
                    if (!validClasses.includes(entry.value)) {
                        this.addWarning(`Unknown robot class '${entry.value}'. Valid: ${validClasses.join(", ")}`, program.meta.span.line, program.meta.span.column);
                    }
                }
            }
        }
    ...
    isResolvable(name) {
        if (this.constants.has(name) || this.stateVars.has(name))
            return true;
